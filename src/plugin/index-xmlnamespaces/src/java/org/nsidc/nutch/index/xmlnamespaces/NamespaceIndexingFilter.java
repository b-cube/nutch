...
/**
 * Description: Extracts xml namespaces into a new Nutch field. Requirements:
 * index-rawxml, index-more
 */
...
public class NamespaceIndexingFilter implements IndexingFilter {
	...
	public NutchDocument filter(NutchDocument doc, Parse parse, Text url,
		...
		// If the raw_content is null or the document type do not contains xml
		// then we just return the document as is.
		String documentType;
		if (doc != null) {
			if (doc.getField("type") != null) {
				documentType = doc.getField("type").getValues().get(0)
						.toString();
				if (doc.getField("raw_content") == null
						|| !documentType.contains("xml")) {
			...
			String raw_xml_content = doc.getField("raw_content").getValues()
					.get(0).toString();
